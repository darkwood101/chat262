...
int main() {
    ...
    assert(c.registration("testuser", "otherpassword", stat_code) ==
           status::ok);
    assert(stat_code == 2);
    ...
    assert(c.registration("A2zpsuE*HbVs", "cQ7Kdtov394x", stat_code) ==
           status::ok);
    assert(stat_code == 4);
    // Registering user with whitespace in username should fail with status code
    // 4
    assert(c.registration("hXGE C3pE pA", "HWLNWEdk9Hed", stat_code) ==
           status::ok);
    assert(stat_code == 4);
    ...
    // Username too short
    assert(c.registration("", "Xj3wMvbJ9cfaKs2zk7L66ENZVT", stat_code) ==
           status::ok);
    assert(stat_code == 4);
    assert(c.registration("aaa", "-X83!C7MmRHyrNAPsehKXx", stat_code) ==
           status::ok);
    assert(stat_code == 4);
